...
# =============================================================================
#
# - stopwords（停用词）是 what/is/the/of 等低信息量词；语料和 query
#   必须共用 tokenize()，两边执行同一套过滤规则。
# - 索引内容是 tokenize(file_path) + tokenize(chunk.text)：路径帮助匹配
#   文件名/模块名，正文负责匹配实际代码或文档内容；这不会修改 Chunk.text。
# - BM25 给“每个 query token × 每个 Chunk”计分并求和，主要考虑 IDF、
#   token 在 Chunk 中的频率、Chunk 长度，以及 k1、b 两个超参数。
# - k1 控制重复词的额外价值；k1 越小，词频越快饱和。这里从默认 1.5
#   改为 1.0，减少同一普通词反复出现对排名的支配。
# - b 控制长度归一化；b=0 完全不考虑 Chunk 长度，b 越大越明显修正
#   长 Chunk。这里从默认 0.75 改为 0.4，减弱对长代码 Chunk 的惩罚。
# - max_chunk_size 决定“如何切”，b 决定“如何按长度评分”，搜索参数 k
#   决定“最终返回几个结果”；三者不是同一个概念。
# - k1=1.0、b=0.4 是手动覆盖 rank_bm25 默认值的实验参数，不是所有
#   数据集的通用最优值；改变分块或语料后应重新用 Recall@k 验证。
